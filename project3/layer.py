...
class Layer():
    ...
    def softmax(self):
        ...
        self.net_act = np.exp(new_net_in) / (np.sum(np.exp(new_net_in), keepdims=True, axis=1))
    ...
    def cross_entropy(self, y):
        ...
        '''
        # re-used from Project 2
        correct = self.net_act[np.arange(self.net_act.shape[0]), y]
    ...
    def backward_netAct_to_netIn(self, d_upstream, y):
        ...
        if self.activation == 'relu':
            ...
            # do a different relational operation if issues
            d_net_in = d_upstream * np.where(self.net_in <= 0, 0, 1)
        
        elif self.activation == 'linear':
            # TODO: compute correct gradient here
            #the change that gets you from netact to netin is nothing
            d_net_in = d_upstream
            
        elif self.activation == 'softmax':
            # TODO: compute correct gradient here
            #tentative 
            y = self.one_hot(y, self.net_act.shape[1])
            d_net_in = d_upstream * self.net_act *(y - self.net_act)
...
class Dense(Layer):
    ...
    def compute_net_in(self):
        ...
        '''
        In = self.input.reshape((self.input.shape[0],-1))
        self.net_in = np.dot(In, self.wts) + self.b
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        # what we have: d_upstream. that's d_net_in.
        
        #print("d_upstream shape", d_upstream.shape)
        #print("netact shape", self.net_act.shape)
        ...
        # 3
        #original d_wts = (d_upstream.T @ self.net_act).T + (self.reg * self.wts)

        #d_upstream (N,H) input_flatten (N, M), d_wts M, H
        
        d_wts = input_flatten.T @ d_upstream + (self.reg * self.wts)  
        

        #print("DEBUG: SHAPES OUT")
        #print("dprev ",dprev_net_act.shape)
        #print("d_wts ",d_wts.shape)
        #print("d_b", d_b.shape)
...
class MaxPooling2D(Layer):
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        upstream: max pooled version of the img
        downstream: has the original dimensions, higher res convolved img ***SELF.INPUT***
        ...
        downstream = np.zeros(shape=(self.input.shape))
        for b in range(mini_batch_sz):
            for n in range(n_chans):
                for y in range(out_y):
                    for x in range(out_x):
                        window = self.input[b, n, y*self.strides:y*self.strides+self.pool_size, x*self.strides:x*self.strides+self.pool_size]

                        most_active = np.argmax(window)
                        ind = list(self.ind2sub(most_active, window.shape))
                        ind[0] += y*self.strides
                        ind[1] += x*self.strides
                        downstream[b, n, ind[0], ind[1]] += d_upstream[b, n, y, x]

        return downstream, None, None
