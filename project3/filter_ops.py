...
def conv2_gray(img, kers, verbose=True):
    ...
    #find thickness of border and pad img with zero.
    border_thickness = int(np.ceil((ker_sz - 1) / 2))
    img_padded = np.pad(img, border_thickness, 'constant')

    filteredImg = np.zeros(shape=(n_kers, img_y, img_x))
    
    for i in range(img_y):
        for j in range(img_x):
            
            #print(f"loop x={i} j={j}")
            #print(f"target:{filteredImg[:,i,j]}")
            #print(f"kers:{kers[:].shape}")
            #print(f"frame: {img_padded[i:i+ker_sz, j:j+ker_sz].shape}")
            #print("DEBUG", np.sum(kers[:] @ img_padded[i:i+ker_sz, j:j+ker_sz],axis=(1,2)).shape)
            
            
            filteredImg[:,i,j] = np.sum((kers[:] * img_padded[i:i+ker_sz, j:j+ker_sz]),axis=(1,2))
...
def conv2(img, kers, verbose=True):
    ...
    ker_sz = ker_x
    #find thickness of border and pad img with zero.
    bt = int(np.ceil((ker_sz - 1) / 2))
   
    #npad: don't pad by any on axis 0, pad by border thickness on each axis after
    npad = ((0,0),(bt,bt),(bt,bt))
    img_padded = np.pad(img, npad, 'constant')

    filteredImg = np.zeros(shape=(n_kers, n_chans, img_y, img_x))
    
    for i in range(img_y):
        for j in range(img_x):
            for k in range(n_kers):
                #print(f"loop x={i} j={j}")
                #print(f"target:{filteredImg[:,:,i,j].shape}")
                #print(f"kers:{kers[:,:,:,np.newaxis].shape}")
                #print(f"frame: {img_padded[:,i:i+ker_sz, j:j+ker_sz].T[np.newaxis,:,:,:].shape}")
                #print("DEBUG", np.sum((kers[:,:,:,np.newaxis] * img_padded[:,i:i+ker_sz, j:j+ker_sz].T[np.newaxis,:,:,:]),axis=(1,2)).shape)
     
                #filteredImg[:,:,i,j] = np.sum((kers[:,:,:,np.newaxis] * img_padded[:,i:i+ker_sz, j:j+ker_sz].T[np.newaxis,:,:,:]),axis=(1,2))
                #filteredImg[:,:,i,j] = np.sum((kers[:,np.newaxis,:,:] * img_padded[:,i:i+ker_sz, j:j+ker_sz]), axis=(1,2))#.T[np.newaxis,:,:,:]),axis=(1,2))
                filteredImg[k,:,i,j] = np.sum(kers[k] * img_padded[:,i:i+ker_sz, j:j+ker_sz], axis=(1,2))
...
def conv2nn(imgs, kers, bias, verbose=True):
    ...
    #npad: don't pad by any on axis 0, pad by border thickness on each axis after
    npad = ((0,0),(0,0),(bt,bt),(bt,bt))
    imgs_padded = np.pad(imgs, npad, 'constant')

    filteredImgs = np.zeros(shape=(batch_sz, n_kers, img_y, img_x))
        ...
        for j in range(img_x):
            for k in range(n_kers):
                for b in range(batch_sz):
                    ...
                    # help from Nhi T.
                    filteredImgs[b,k,i,j] = np.sum(kers[np.newaxis, k] * imgs_padded[b,:,i:i+ker_sz, j:j+ker_sz], axis=(1,2,3)) + bias[k]
...
def max_pool(inputs, pool_size=2, strides=1, verbose=True):
    ...
    if verbose:
        print(f'out_y={out_y}, out_x={out_x}')

    outputs = np.zeros(shape=(out_y, out_x))
    
    for i in range(out_y):
        for j in range(out_x):
            # help w/strides and pool size from Nhi T.
            outputs[i,j] = np.max(inputs[i*strides:i*strides+pool_size, j*strides:j*strides+pool_size])
...
def max_poolnn(inputs, pool_size=2, strides=1, verbose=True):
    ...
    pool_size: int. Pooling window extent in both x and y.
    strides: int. How many "pixels" in x and y to skip over between successive max pooling operations
    verbose: bool. I suggest making helpful9+
 print statements showing the shape of various things
        as you go. Only execute these print statements if verbose is True.
    ...
    outputs = np.zeros(shape=(mini_batch_sz, n_chans, out_y, out_x))
        ...
        for j in range(out_x):
            for b in range(mini_batch_sz):
                for c in range(n_chans):
                    # help w/strides and pool size from Nhi T.
                    outputs[b,c,i,j] = np.max(inputs[b,c,i*strides:i*strides+pool_size, j*strides:j*strides+pool_size])
