...
class DeepDream():
    ...
    def forward(self, img_tf, verbose=False):
        ...
        #adding batch dim to the img
        img_tf = tf.expand_dims(img_tf, axis=0)

        netacts_at_each_layer = self.net(img_tf)

        selected_netacts = [netacts_at_each_layer[i] for i in self.selected_layer_inds]
        
        avgs = []
        if len(self.filter_inds) > 0:
            #we need to fo specific indices.
            for i in range(len(selected_netacts)):
                selected_netacts[i]= tf.gather(params=selected_netacts[i], indices=self.filter_inds, axis=3)
                avgs.append(tf.math.reduce_mean(selected_netacts[i]))
        else:
            #we can average all of the netacts indiscriminately
            for each in selected_netacts:
                avgs.append(tf.math.reduce_mean(each))
    ...
    def image_gradient(self, img_tf, eps=1e-8, normalized=True, verbose=False):
        ...
        with tf.GradientTape(persistent=True) as g:
            g.watch(img_tf) 
            net_acts = self.forward(img_tf)


        grads = [] 
        for i in range(len(net_acts)):
            grads.append(g.gradient(net_acts[i], img_tf))
        ...
        if normalized:
            for i in range(len(grads)):
                top = tf.math.subtract(grads[i], tf.reduce_mean(grads[i]))
                bot = tf.math.reduce_std(grads[i]) + tf.constant(eps)
                grads[i] = tf.math.divide(top, bot)
    ...
    def gradient_ascent(self, img_tf, n_iter=10, step_sz=0.01, clip_low=0, clip_high=1, verbose=False):
        ...
        for i in range(n_iter):
            if verbose:
                print(f"Iteration {i}")
            grads = self.image_gradient(img_tf, normalized=True, verbose=verbose)
            for each in grads:
                img_tf = img_tf + each*step_sz
 
 
 
        return tf.clip_by_value(img_tf, clip_low, clip_high)
    ...
    def gradient_ascent_multiscale(self, img_tf, n_scales=4, scale_factor=1.3, n_iter=10, step_sz=0.01,
        ...
        for n in range(n_scales):
            if verbose:
                print(f"Multiplicative iteration {n+1} / {n_scales}")
            ...
            if verbose:
                print(f"Image shape is {img_tf.shape}")
    ...
    def tf2array(self, tf_img):
        ...
        np_img = np.array(tf_img)
        np_img = (np_img - np_img.min())/np_img.max()
        return np_img
